...
#datafile <- mutate(datafile, RDDsameasDD = case_when(dmy(discharge_date) == dmy(date_declared_medically_fit) ~ 1))
#datafile <- filter(datafile, dmy(discharge_date)==dmy(date_declared_medically_fit))

class(datafile$discharge_date)
class(datafile$date_declared_medically_fit)
table(datafile$RDDsameasDD)

#datafile$RDDsameasDD <- ifelse(datafile$discharge_date == date_declared_medically_fit,1,0)

datafile <- mutate(datafile, RDDsameasDD = 
                 if_else(dmy(datafile$discharge_date) == dmy(datafile$date_declared_medically_fit), 1, 0))
#datafile_test<-filter(datafile,dmy(discharge_date)==dmy(date_declared_medically_fit))        
