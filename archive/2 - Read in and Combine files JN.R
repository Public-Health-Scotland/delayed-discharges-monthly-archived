...
#table(Scotland_allboards$NHS.Board)


### 9. Save out file as csv and also as a sav file  ----

write_csv(Scotland_allboards,"//conf/delayed_discharges/RAP development/2019_07/Outputs/Allboards_R_JN_.csv")
write_sav(Scotland_allboards,"//conf/delayed_discharges/RAP development/2019_07/Outputs/Allboards_R_JN.sav")
#View(Scotland_allboards)
Allboardscheck <- read_sav("//conf/delayed_discharges/RAP development/2019_07/Outputs/Allboards_R_JN.sav")

###10. Ensure that the labels show in the output file  ----
Allboardscheck <- Allboardscheck %>%
  new_hb_label = attr(nhs_board, "names")

dictdf <- haven::read_spss(paste("//conf/delayed_discharges/RAP development/2019_07/"("//conf/delayed_discharges/RAP development/2019_07/Outputs/", "ReferenceFiles", "Dictionary_School.sav")))
fulldatadic <- tibble(
  name = colnames(dictdf),
  new_label = dictdf %>%
    purrr::map(~ attr(`nhs_board`, "label")) %>%
    unlist())

### 11. Check that Allboards_R.sav matches original spss Allboards.sav file.  ----
# (saving out spss output as a csv to check against R output)

Allboardsspss <- read_sav("//conf/delayed_discharges/RAP development/2019_07/Data/scotland/AllBoards.sav")

View(Allboardsspss)
write_csv(Allboardsspss,"//conf/delayed_discharges/RAP development/2019_07/Outputs/Allboards_SPSS_JN_.csv")